(ns tae-technologies.main
  "Kotodama WASM entrypoint for the Tae_technologies clean-room actor (L4) — Clojure port.
  ...
  facts (`tae_technologies.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tae_technologies")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tae_technologies.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tae_technologies-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tae_technologies-compat/ready)
